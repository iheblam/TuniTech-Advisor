...
class DataService:
    ...
    def _load_data(self):
        ...
        try:
            ...
            print(f"[OK] Data loaded successfully: {len(self.data)} smartphones")
            
            # Clean column names (remove leading/trailing spaces)
            self.data.columns = self.data.columns.str.strip()
            
            # Normalize column names to match API schema field names
            column_map = {
                'ram_gb': 'ram',
                'storage_gb': 'storage',
                'battery_mah': 'battery',
                'screen_inches': 'screen_size',
                'camera_rear_mp': 'main_camera',
                'camera_front_mp': 'front_camera',
                'processor_type': 'processor',
                'source': 'store',
            }
            self.data.rename(columns={k: v for k, v in column_map.items() if k in self.data.columns}, inplace=True)

            # Derive is_5g boolean from network column if present
            if 'network' in self.data.columns and 'is_5g' not in self.data.columns:
                self.data['is_5g'] = self.data['network'].str.upper().str.contains('5G', na=False)

            # Add has_nfc column defaulting to False if not present
            if 'has_nfc' not in self.data.columns:
                self.data['has_nfc'] = False

            # Add sequential id column if not present
            if 'id' not in self.data.columns:
                self.data.insert(0, 'id', range(1, len(self.data) + 1))

            print(f"[OK] Columns normalised: {list(self.data.columns)}")
